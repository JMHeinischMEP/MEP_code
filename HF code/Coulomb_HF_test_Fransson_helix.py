...
#%%
from Hexagon_construct_Fransson_functions_only import index_convert, rev_index_convert, generate_connections, Atom_mult_pos
...
revI_C[:,Conn_H][:,0,0]
Pos_A = Atom_mult_pos(revI_C, 1, N_hex_unit)

Pos_A_org = Pos_A[:,I_C][:,2:,:]  #Such that first site is on the y-axis (0,1,0)
Pos_A_org[2] += -Pos_A_org[2,2,0]
#Pos_A_org[0] = np.copy(-Pos_A_org[0])

DiffPos1 = np.diff(Pos_A_org[:,:,0], axis = 1)
DiffPos_len1 = np.sqrt(np.sum(DiffPos1**2, axis=0))
DiffPos_norm1 = DiffPos1/DiffPos_len1

DiffPos2 = Pos_A_org[:,:-2,0] - Pos_A_org[:,2:,0]
DiffPos_len2 = np.sqrt(np.sum(DiffPos2**2, axis=0))
DiffPos_norm2 = DiffPos2/DiffPos_len2
...
Cross_diff = np.cross(DiffPos_norm1[:,:-1], DiffPos_norm2[:,:], axisa = 0, axisb = 0) 
