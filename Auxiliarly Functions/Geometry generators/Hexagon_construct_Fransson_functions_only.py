...
Phi1 = 45*np.pi/180#60*np.pi/180
Phi2 = 19.1*np.pi/180
...
def Z_ratio_mult_site(Ind_arr, N_hex_unit):
    """
    Gives the angles of the indices
    Input:
        Ind_arr : Indices, where Ind_arr[0] stores the hexagons' indices, while Ind_arr[1] stores the atoms' indices
    ...
    Ind_Hex = Ind_arr[0]
    Ind_Atom = Ind_arr[1]
    del_z_helix = 1/(N_hex_unit - 1)
    del_z_vals = 1/Phi1*np.array([0,0,Phi2,Phi1-Phi2])/(N_hex_unit - 1)
    z_vals_tot = Ind_Hex*del_z_helix + del_z_vals[Ind_Atom]
    return z_vals_tot
...
def Atom_mult_pos(Ind_arr, pitch, N_hex_unit = 8):
    """
    ...
    Z_values = pitch*Z_ratio_mult_site(Ind_arr, N_hex_unit)
    Spiral_indices = Spiral_mult_map(Ind_arr)
    Radius_dict = np.array([R_I,R_M,R_A])
    Radius_vals = Radius_dict[Spiral_indices]
    Pos = np.array([Radius_vals*np.cos(Angles), Radius_vals*np.sin(Angles), Z_values])
    return Pos
